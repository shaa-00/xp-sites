...
Currently configured project: `xp-sites` (target: `..`).
...
original init command. Checkpoints in `data/xp-sites/setup/setup-state.json`
...
`--project-id xp-sites` (or whichever id you want) explicitly.

`scan` is free (regex only). `process` is the AI stage (≈$0.30/file
on Opus by default). Run state goes to `data/xp-sites/`.
