# Agent setup for `xp-sites`

This is a deepsec scanning workspace. Project `xp-sites` was just registered
(target: `..`). Setup is incomplete — `data/xp-sites/INFO.md`
...
2. **Fill in `data/xp-sites/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id xp-sites
pnpm deepsec process --project-id xp-sites
```
